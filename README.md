...
[index.html @ GitHub Pages]
  锁屏输入同步密码 → 本地解出同步凭证（驻留内存，锁定/刷新即清）
        -- GET git/trees?recursive=1 列出所有 .md 文档 -->
        -- GET/PUT/DELETE contents/{path} 打开 / 保存 / 删除 .md 文件 -->
[本仓库 @ main 分支] 直接多出 / 更新 / 删除笔记
```

提交信息统一为 `同步笔记：<路径>` / `删除笔记：<路径>`。`overwrite` 未勾选且文件已存在时拒绝写入。
...
1. **同步凭证**：已固定写死在 `index.html` 顶部 `SYNC_CREDENTIAL`，页面无任何修改入口。
  ...
  （读用于列目录 / 查 sha / 打开文件，写用于创建、更新、删除），有效期尽量短。
...
* **密码锁屏**：进入页面需输入同步密码，正确后自动列出仓库文档；顶部「锁定」可退出并清除内存凭证
* **文档管理**：列出全部笔记（路径 + 大小），点击可打开编辑，支持删除（删除前二次确认）
* 编辑/预览双 tab（自带轻量 Markdown 渲染，先转义防 XSS）、字数/行数/阅读时长
* 草稿自动保存（localStorage，仅本机）、一键插入模板/清空草稿
* 子目录 + 标题 + 覆盖开关 + 空标题按日期自动命名；`Ctrl/⌘+Enter` 保存
* 明暗主题
